...
const CartSlot = ({ index, image, name, size, color, price, numberOfUnits, removeItem, minusUnits, plusUnits }) => {
    ...
    return (
        <div className='h-24 xl:w-96 w-80 py-4 flex justify-between'>
            ...
            <div className='flex flex-col justify-between items-end'>
                <FontAwesomeIcon icon={faTrashCan} className='text-[#e65a5a] cursor-pointer' onClick={removeItem}/>
                <div className='bg-[#F0F0F0] rounded-[3.875rem] flex gap-3 items-center py-[0.25rem] px-[0.75rem]'>
                    <img src={minus.src} alt="minus" className='w-4 h-4 cursor-pointer' onClick={minusUnits}/> <span className='cursor-default'>{numberOfUnits}</span> <img src={plus.src} alt="plus" className='w-4 h-4 cursor-pointer' onClick={plusUnits}/>
                </div>
            </div>
        </div>
    );
